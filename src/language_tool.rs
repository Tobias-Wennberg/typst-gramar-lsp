...
use std::str::Chars;
pub async fn check_text(typst_text :&String) -> TextCheck {
	...
	for items in converted_nodes {
        ...
        let (mut diagnostics, chunk) = add_chunk(&mut position, response, items.1);
        text_check.diagnostic.append(&mut diagnostics);
        text_check.matches.push(chunk);
	}
...
fn add_chunk(
    ...
    total: usize,
    ) -> (Vec<Diagnostic>, MatchChunk){
    let mut last = 0;
    let mut diagnostic_data :Vec<Diagnostic> = vec![];
    let mut chunk = MatchChunk {
        language_code: response.language.code.clone(),
        matches: vec![]
    };
	for info in &response.matches {
        ...
        chunk.matches.push(
            Match {
                match_data: info.clone(),
                range: r
            });
		last = info.offset;
	}
	start.advance(total - last);
    return (diagnostic_data, chunk);
